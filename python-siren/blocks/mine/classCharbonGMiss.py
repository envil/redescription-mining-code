...
class CharbonGMiss(CharbonGreedy):
    ...
    def getCandidates(self, side, col, red, colsC=None, data=None):
        ...
        if colsC is not None and self.constraints.getCstr("add_condition"):
            for c in self.store.currentRoundIter():
                ...
                sI = ss.part_union(self.constraints.getSSetts().IDS_inter)
                sD = ss.part_union(self.constraints.getSSetts().IDS_diff)
                cond_sparts = SParts(self.constraints.getSSetts(), ss.nbRows(), [sI, sD.union(sI)])
    ...
    def getCandidates1(self, side, col, supports, currentRStatus=0):
        ...
        for op in self.constraints.getCstr("allw_ops", side=side, currentRStatus=currentRStatus):
            for neg in self.constraints.getCstr("allw_negs", side=side, type_id=col.typeId(), currentRStatus=currentRStatus):
                adv, clp, stt = self.getAC(side, op, neg, [lparts, lmiss, lin], self.isCond(currentRStatus))
                if adv is not None:
    ...
    def getCandidatesNonBool(self, side, col, supports, currentRStatus=0):
        lparts = supports.lparts()
        lmiss = supports.lpartsInterX(col.miss())
        self.findCover(side, col, lparts, lmiss, supports, currentRStatus)
    ...
    def getConditionCand(self, colsC, cond_sparts, lparts):
        ...
        while len(cis) > 0:  # if there are multiple condition attributes, iteratively and greedily try to find a combination
            ...
            for ci in cis:
                ...
                self.findCover(1, colsC[ci], lparts, lmiss, cond_sparts, Constraints.getStatusCond())
    ...
    def findCover(self, side, col, lparts, lmiss, supports, currentRStatus=0):
        ...
        method_compute(side, col, lparts, lmiss, supports, currentRStatus)
    ...
    def findCover1(self, side, col, lparts, lmiss, supports, currentRStatus=0):
        ...
        for op in self.constraints.getCstr("allw_ops", side=side, currentRStatus=currentRStatus):
            for neg in self.constraints.getCstr("allw_negs", side=side, type_id=col.typeId(), currentRStatus=currentRStatus):
                ...
                if self.constraints.getCstr("neg_query_init", side=side, currentRStatus=currentRStatus):
                    adv, clp, stt = self.getAC(side, op, neg, [nlparts, nlmiss, nlin], self.isCond(currentRStatus))
                    if adv is not None:
        ...
        for op in self.constraints.getCstr("allw_ops", side=side, currentRStatus=currentRStatus):
            for neg in negs:
                ...
                for (cat, supp) in col.iter_cats():
                    ...
                    tmp_adv, tmp_clp, stt = self.getAC(side, op, neg, [lparts, lmiss, lin], self.isCond(currentRStatus))
                    ...
                    if self.constraints.getCstr("neg_query_init", side=side, currentRStatus=currentRStatus):
                        nlin = self.constraints.getSSetts().negateParts(1-side, lin)
                        tmp_adv, tmp_clp, stt = self.getAC(side, op, neg, [nlparts, nlmiss, nlin], self.isCond(currentRStatus))
                        if tmp_adv is not None:
    ...
    def findCover3(self, side, col, lparts, lmiss, supports, currentRStatus=0):
        ...
        for op in self.constraints.getCstr("allw_ops", side=side, currentRStatus=currentRStatus):
            ...
            if self.constraints.isStatusCond(currentRStatus) or self.inSuppBounds(side, op, lparts):  # DOABLE
                ...
                self.findCoverSegments(side, op, col, segments, counts, lparts, lmiss, currentRStatus, nego=False)
            ...
            if self.constraints.getCstr("neg_query_init", side=side, currentRStatus=currentRStatus):
                ...
                if self.inSuppBounds(side, op, nlparts):  # DOABLE (op is True, OR, for initial pairs)
                    ...
                    self.findCoverSegments(side, op, col, nsegments, ncounts, nlparts, nlmiss, currentRStatus, nego=True)

    def findCoverSegments(self, side, op, col, segments, counts, lparts, lmiss, currentRStatus=0, nego=None):
        if len(segments) < self.constraints.getCstr("max_seg"):
            self.findCoverFullSearch(side, op, col, segments, counts, lparts, lmiss, currentRStatus, nego)
        else:
            if (False in self.constraints.getCstr("allw_negs", side=side, type_id=col.typeId(), currentRStatus=currentRStatus)):
                self.findPositiveCover(side, op, col, segments, counts, lparts, lmiss, currentRStatus, nego)
            if (True in self.constraints.getCstr("allw_negs", side=side, type_id=col.typeId(), currentRStatus=currentRStatus)):
                self.findNegativeCover(side, op, col, segments, counts, lparts, lmiss, currentRStatus, nego)

    def findCoverFullSearch(self, side, op, col, segments, counts, lparts, lmiss, currentRStatus=0, nego=None):
        ...
        for seg_si, seg_s in enumerate(segments):
            for seg_e in segments[seg_si:]:
                ...
                for neg in self.constraints.getCstr("allw_negs", side=side, type_id=col.typeId(), currentRStatus=currentRStatus):
                    bests[neg], stt = self.updateACT(bests[neg], (seg_s[0], seg_e[1]), side, op, neg, [lparts, lmiss, lin], self.isCond(currentRStatus))
    ...
    def findPositiveCover(self, side, op, col, segments, counts, lparts, lmiss, currentRStatus=0, nego=None):
        ...
        for i, seg_f in enumerate(segments[:-1]):
            ...
            if i > 0 and cmp_lower(self.advAcc(side, op, False, [lparts, lmiss, lin_segf], is_cond), self.advRatioVar(side, op, lin_f, is_cond)):
            ...
            best_f, stt = self.updateACT(best_f, tuple(bound_ids_f), side, op,
                                         False, [lparts, lmiss, list(lin_f)], is_cond)
            ...
            if i > 0 and cmp_lower(self.advAcc(side, op, False, [lparts, lmiss, lin_segb], is_cond),
            ...
            best_b, stt = self.updateACT(best_b, tuple(bound_ids_b), side, op,
                                         False, [lparts, lmiss, list(lin_b)], is_cond)
        ...
        if best_b[0] is not None and best_f[0] is not None:
            ...
            if cmp_greater(best_b[-1][-1][0], best_f[-1][-1][0]) and \
                ...
                tmp_adv_m, tmp_clp_m, stt = self.getAC(side, op, False, [lparts, lmiss, lin_m], is_cond)
    ...
    def findNegativeCover(self, side, op, col, segments, counts, lparts, lmiss, currentRStatus=0, nego=None):
        ...
        lin_f = numpy.zeros(counts.shape[1], dtype=int)
        bests_f = [(self.advAcc(side, op, False, [lparts, lmiss, lin_f], is_cond), -1, lin_f)]
        lin_b = numpy.zeros(counts.shape[1], dtype=int)
        bests_b = [(self.advAcc(side, op, False, [lparts, lmiss, lin_b], is_cond), -1, lin_b)]
        ...
        for i, seg_f in enumerate(segments):
            ...
            if cmp_greater(self.advRatioVar(side, op, lin_f, is_cond), bests_f[-1][0]):
                lin_f += bests_f[-1][2]
                bests_f.append((self.advAcc(side, op, False, [lparts, lmiss, lin_f], is_cond), seg_f[0], lin_f))
                lin_f = numpy.zeros(counts.shape[1], dtype=int)
            ...
            if cmp_greater(self.advRatioVar(side, op, lin_b, is_cond), bests_b[-1][0]):
                lin_b += bests_b[-1][2]
                bests_b.append((self.advAcc(side, op, False, [lparts, lmiss, lin_b], is_cond), seg_b[1], lin_b))
                lin_b = numpy.zeros(counts.shape[1], dtype=int)
        ...
        while ixf < len(bests_f) and ixb < len(bests_b):
            if ixf+1 == len(bests_f) or bests_b[ixb][1] < bests_f[ixf+1][1]:
                best_t, stt_b = self.updateACT(best_t, (None, bests_b[ixb][1]), side, op,
                                               False, [lparts, lmiss, bests_b[ixb][2]], is_cond)
                ...
                if ixf > -1 and not self.is_raised_flag(stt_b, self.FLG_OUT):
                    best_t, stt_p = self.updateACT(best_t, (bests_f[ixf][1], bests_b[ixb][1]), side, op,
                                                   False, [lparts, lmiss, bests_b[ixb][2] + bests_f[ixf][2]], is_cond)
                    ...
                    #                                       False, [lparts, lmiss, bests_b[ixb][2] + bests_f[off_f][2]], is_cond)
                ...
                best_t, stt_f = self.updateACT(best_t, (bests_f[ixf][1], None), side, op,
                                               False, [lparts, lmiss, bests_f[ixf][2]], is_cond)
    ...
    def fit(self, col, supports, side, termX):
        ...
        self.findCover(side, col, lparts, lmiss, supports, currentRStatus=currentRStatus)
        for c in self.store.currentRoundIter():
            ...
            while lowF < len(interMat) and totInt - belowF >= self.constraints.getCstr("min_itm_in"):
                ...
                while upF >= lowF and totInt - belowF - aboveF >= self.constraints.getCstr("min_itm_in"):
                    if belowF + aboveF >= self.constraints.getCstr("min_itm_out"):
                        ...
                        lparts = self.constraints.getSSetts().makeLParts([totInt - aboveF - belowF + lmissE,
                                                                          aboveF + belowF + totMissEinF - lmissE,
                                                                          partsMubB], side=side)
                        lmiss = self.constraints.getSSetts().makeLParts([lmissE,
                                                                         totMissEinF - lmissE,
                                                                         missMubB], side=side)
                        belowEF = 0
                        outBelowEF = 0
                        ...
                        while lowE < len(interMat[lowF]) and totInt - belowF - aboveF - belowEF >= self.constraints.getCstr("min_itm_in"):
                            ...
                            while upE >= lowE and totInt - belowF - aboveF - belowEF - aboveEF >= self.constraints.getCstr("min_itm_in"):
                                ...
                                stt = self.updateACTList(33, best, (lowF, upF, lowE, upE), side, True, False, [lparts, lmiss, lin])
                                aboveEF += EinF[upE]
        ...
        for catL in colL.cats():
            ...
            for catR in colR.cats():
                ...
                for i, (nL, nR) in enumerate(configs):
                    ...
                    stt = self.updateACTList(22, best[i], (catL, catR), side, True, nR, [tmp_lparts, tmp_lmiss, tmp_lin], pre_multi=self.constraints.getCstr("multi_cats"))
    ...
    def subdo23Full(self, colL, colR, side, try_comb=True):
        ...
        if buckets is not None and (len(buckets[1]) * len(colF.cats()) <= self.constraints.getCstr("max_prodbuckets")):
            ...
            for cat in colF.cats():
                ...
                lmiss = self.constraints.getSSetts().makeLParts([lFcEm,
                    ...
                    while up >= low and \
                        ...
                        for i, (nF, nE) in enumerate(configs):
                            if nF:
                                ...
                                tmp_lmiss = self.constraints.getSSetts().negatePartsV(1-side, lmiss)
                            ...
                            stt = self.updateACTList(23, best[i], (cat, low, up), side, True, nE, [tmp_lparts, tmp_lmiss, tmp_lin], pre_multi=multi_cats)
    ...
    def checkCountsPair(self, side, neg, colF, colE, tF, tE, clp):
        ...
        rv = (org_ltot == new_ltot) and (org_lin == new_lin) and (org_lmiss == new_lmiss)
    ...
    def checkCountsExt(self, supports, col, c):
        ...
        lout = [lparts[i]-lin[i]-lmiss[i] for i in range(len(lparts))]
