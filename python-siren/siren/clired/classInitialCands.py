...
def fifo_sort(initc_store, initc_details, drop_set=set(), sort_criterion="score"):
    return sorted(drop_set.symmetric_difference(initc_store.keys()), reverse=True)

def filo_sort(initc_store, initc_details, drop_set=set(), sort_criterion="score"):
    return sorted(drop_set.symmetric_difference(initc_store.keys()))

def overall_sort(initc_store, initc_details, drop_set=set(), sort_criterion="score"):
    return sorted(drop_set.symmetric_difference(initc_store.keys()), key=lambda x: initc_details[x].get(sort_criterion, DEF_SCORE))

def alternate_sort(initc_store, initc_details, drop_set=set(), sort_criterion="score"):
    best_sides = [{}, {}]
    for k in initc_store.keys():
        for side in [0,1]:
            col = initc_details[k].get(side, -1)
            if col >= 0:
    ...
    for side in [0,1]:
        for col in best_sides[side]:
            best_sides[side][col].sort(key=lambda x: initc_details[x].get(sort_criterion, DEF_SCORE), reverse=True)
        for c, vs in best_sides[side].items():
            for pp, v in enumerate(vs):
                initc_details[v]["rank_%d"%side] = pp
    ord_ids = sorted(drop_set.symmetric_difference(initc_store.keys()), key=lambda x: initc_details[x].get("score")-max(initc_details[x].get("rank_0", -1), initc_details[x].get("rank_1", -1)))
    return ord_ids
...
class InitialCands(object):
        ...
        if self.save_filename is not None and not self.saved:
            try:
                with open(self.save_filename, "w") as f:
                    for pi, p in self.initc_store.items():
                        f.write("%s\t%s\n" % (p[0], p[1]))
    ...
    def loadFromLogFile(self, data=None):
        ...
        end_patt = log_patt+"Found \d+ init cands,"
    ...
    def reset(self):
        self.list_out = []
        self.drop_set = set()
        self.initc_store.clear()
        self.initc_details.clear()
        self.sorted_ids = None
        self.setExploreList()
        self.saved = False

    def __len__(self):
        return len(self.initc_store)
    ...
    def __str__(self):
        return "Initial Cands %d" % len(self.initc_store)

    def add(self, literalL, literalR, details={}):       
        self.saved = False
        self.initc_store[self.next_id] = (literalL, literalR)
        if details is not None and len(details) > 0:
            self.initc_details[self.next_id] = details
        else:
            self.initc_details[self.next_id] = self.next_id
        self.next_id += 1
        self.sorted_ids = None

    def _sort(self):
        if self.sorted_ids is None:
            self.sorted_ids = self.sort_meth(self.initc_store, self.initc_details, self.drop_set, self.sort_criterion)
        
    def pop(self, cond=None):
        if len(self.initc_store) > 0 and not self.exhausted():
            self._sort()
            if len(self.sorted_ids):
                nid = self.sorted_ids.pop()
                tt = self.initc_store[nid]
                dt = self.initc_details[nid]
                self.drop_set.add(nid)
                if cond is not None:
                    while not cond(tt) and len(self.initc_store) > 0:
                        nid = self.sorted_ids.pop()
                        tt = self.initc_store[nid]
                        dt = self.initc_details[nid]
                        self.drop_set.add(nid)
                    if not cond(tt) and len(self.initc_store) == 0:
                        return None, None
    ...
    def getAllTerms(self, data, cond=None, min_score=None):
        terms = []
        scs = []
        for nid, dt in self.initc_details.items():
            if min_score is None or dt[self.sort_criterion] >= min_score:
                pair = self.initc_store[nid]
                terms.append(pair)
                scs.append(dt[self.sort_criterion])
        return terms, scs
...
def initCands(sort_meth="overall", max_out=-1, save_filename=None, data=None, mdl=False):
    return InitialCands(sort_meth, max_out, save_filename)
