...
setts    static recursion/output setts as a list
    ...
    for k in range(len(tadb)):  # traverse the items/item sets
        ...
        for r,j,u in tadb[k+1:]:# trans. settsbase to the current item:
            ...
            elif r >= setts[1]: proj.append([r,j,u])
        ...
        if   setts[0] == 'm':    # if to report only maximal item sets
            ...
            if r < s: report(xset+xpxs, [], s, setts, store_handle)
...
class CharbonXLevel(CharbonXaust):

    name = "XaustLevel"

    def isLevelwise(self):
        return True
    ...
    def computeExpansions(self, data, initial_candidates):
        ...
        cs = CandStore(cs_setts)
        r = mod_eclat(tracts, ['s', min_supp, zmin, zmax, zmax+1], cs.add)
        queries = cs.getQueries()
        cands = []
        for qs in queries:
            lits0 = [initial_candidates[map_initc[0][q[1]][q[2]]][0] for q in qs[0]]
            lits1 = [initial_candidates[map_initc[1][q[1]][q[2]]][1] for q in qs[1]]
