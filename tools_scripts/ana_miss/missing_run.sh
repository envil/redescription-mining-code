...
RESULTS_REP=${BASE_REP}results_missing/
...
OTHER_VAR_PARAMS='::DATA_REP::='$DATA_REP',::RESULTS_REP::='$RESULTS_REP',::EXT_RULES::='$EXT_RULES',::EXT_L::='$EXT_L',::EXT_R::='$EXT_R',::SUFF_DATA::='$SUFF_MISS',::CONTRIBUTION::='$CONTRIBUTION',::MIN_IMPROV::='$(( $FULL_BOOL * -1 ))
...
for l = 1:length(confs)
     for i=1:confs(l).number
         n = 8005*full(sprand(size(L,1), size(L,2),confs(l).density) > 0);
	 L=L-n;
	 n = 8005*full(sprand(size(R,1), size(R,2),confs(l).density) > 0);
	 R=R-n;
...
"
 	...
 	echo "Generating missing ${SERIES} ..."
	echo "${SCRIPT_MATLAB}" | $MATLAB_BIN -nosplash -nodesktop > /dev/null

	sed -i 's/-800[0-9]/m/g' ${DATA_REP}L*_$SERIES_$SUFF_MISS*
	sed -i 's/-800[0-9]/m/g' ${DATA_REP}R*_$SERIES_$SUFF_MISS*
...
#	   echo "LINE_OUT" $LINE_OUT
 	    /usr/bin/time -f "$TIME_FORMAT" -o ${OUT}$EXT_TIME  $MINE_SCRIPT $MINE_CONF '::SERIES::='$TYPE'_'$SERIES','$OTHER_VAR_PARAMS	
