...
for copy in $(ls ${RES_REP}*_missing.compare ) 
do
    #echo "COPY " $copy
    ...
    ORG_QUERY=$( tail -1 ${RES_REP}${TYPE}_${SERIES}_missing.compare )
    ORG_MATCH=$( sed '$d' ${RES_REP}${TYPE}_${SERIES}_missing.compare | grep "$ORG_QUERY" - )
    if [ -n "$ORG_MATCH" ]; then
	echo "MATCH---"
	MATCH=1
    else
    ...
    sed '$d' ${RES_REP}${TYPE}_${SERIES}_missing.compare | cut -f 3,4 |  sed "s/^ *\([0-1].[0-9]* [0-1].[0-9]*\) .*# *\([0-1]\.[0-9]* [0-1].[0-9]*\) .*$/${TYPE} ${SERIES} \1 \2/g" >>  ${OUT}
done
