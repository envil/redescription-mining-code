...
def VoronoiLineEdges(PointsMap):
  ...
  return vertices, lines, edges, has_edge, stids
...
def VoronoiPolygons(PointsMap, BoundingBox="W", PlotMap=False):
  ...
  if (PlotIt):
    for station, data in voronoi_lattice.items():
      ...
      plt.plot(data["coordinate"][0],data["coordinate"][1], "ro")
      print data["coordinate"][0],data["coordinate"][1],data["info"]
      plt.text(data["coordinate"][0],data["coordinate"][1],data["info"])
...
def VoronoiPolygonsMod(PointsMap, BoundingBox="W", PlotMap=False):
  global PlotIt
  global WorldRange
  global WorldRanges
