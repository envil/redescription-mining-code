        ...
        for l = 1:length(confs)
            for j = 0:params.differents-1
                [L, R, acc] = synthetic_boolean_data_pair(setts(i).nb_rows, setts(i).nb_cols_L, setts(i).nb_cols_R, ...
                            setts(i).supp_rows_L, setts(i).supp_rows_R, setts(i).nb_variables_L, setts(i).nb_variables_R, confs(l).OR_L, confs(l).OR_R, setts(i).offset, ...
                ...
                to_log_setts = [i, l, j, acc, setts(i).nb_variables_L, setts(i).nb_variables_R, confs(l).OR_L, confs(l).OR_R, setts(i).offset, ...
				 setts(i).supp_rows_L, setts(i).supp_rows_R, setts(i).preserving,  setts(i).margin_L, setts(i).margin_R, setts(i).density, setts(i).density_blurr_OR, setts(i).density_blurr_AND];
...
function [L, R, acc] = synthetic_boolean_data_pair( nb_rows, nb_cols_L, nb_cols_R, supp_rows_L, supp_rows_R, nb_variables_L, nb_variables_R, opOR_L, opOR_R, offset, preserving, margin_L, margin_R, density, density_blurr_OR, density_blurr_AND)
    ...
    %% skeleton    
    if opOR_L
        [ NL, ML, QL ] = makeRuleSkeleton(nb_variables_L,  supp_rows_L-nb_variables_L, nb_rows-supp_rows_L, density_blurr_OR, false);
    else
        [ NL, QL, ML ] = makeRuleSkeleton(nb_variables_L,  nb_rows-supp_rows_L-nb_variables_L, supp_rows_L, density_blurr_AND, true);
    end

    if opOR_R
        [ NR, MR, QR ] = makeRuleSkeleton(nb_variables_R,  supp_rows_R-nb_variables_R, nb_rows-supp_rows_R, density_blurr_OR, false);
    else
        [ NR, QR, MR ] = makeRuleSkeleton(nb_variables_R,  nb_rows-supp_rows_R-nb_variables_R, supp_rows_R, density_blurr_AND, true);
    end
    ...
    if opOR_L
        ...
        if opOR_R
            ...
            if nb_variables_L > size(QR,1)
                error('QR too small !')
            end
            rulec_L = [ML; QL(:,end:-1:1); NL];
            rulec_R = [QR(nb_variables_L + [1:offset],:); MR; QR(nb_variables_L + 1 + offset:end,:); NR; QR(1:nb_variables_L,:)];
        end
    ...
    if margin_L < 1
        L =abs(L-((1-max(margin_L, eps))/2*rand(size(L))));
    end
    if margin_R < 1
        R =abs(R-((1-max(margin_R, eps))/2*rand(size(R))));  
    end
...
function [ N, M, Q ] = makeRuleSkeleton(rows_N,  rows_M, rows_Q, density_add, invert)    
