        ...
        for idL in ids[0]:
            ...
            for idR in ids[1]:
                ...
                for i in range(len(scores)):
                    ...
                    if redE.pVal() < constraints.maxPVal() and redE.acc() > 0.02:
                        tmpCurrGen.append((redE.queries, {'acc': redE.acc(), 'opacc': redE.opacc()}))
    ...
    def kidsPair(self, currGen, extKeys, idsKey, parents, parts_list, constraints, tmpCurrGen, tmpExtKeys):
        ...
        for alternative in alternatives:
            ...
            for (ext, redI) in alternative:
                ...
                for side in [0,1]:
                    highOp[side].append(currGen[redI][0][side].highOp())
                    lowOp[side].append(currGen[redI][0][side].lowOp())
            ...
            if len(sExt) == 2:
                ...
                for i in range(len(sideExt)):
                    queries[1-sideExt[i]].append(currGen[alternative[i][1]][0][1-sideExt[i]])
                ...
                if len(shOps) == 1 and 0 in shOps:
                    ...
                    if len(slOps) == 1:
			(sideC, colC, negC) = Redescription.decoInvStr(alternative[0][0])
                        tmp = currGen[alternative[1][1]][0][sideC].searchTerm(colC, negC)
                        if tmp != None:
			    tmp.update({'side': sideC, 'col': colC})
			    redP = Redescription.fromQueriesPair(currGen[alternative[0][1]][0], self)
                            redE = redP.cousin(self, tmp['side'], tmp['buk'], tmp['term'])
                        ...
                        while i < len(lowOp[side]) and lowOp[side][i] == opNum:
                            i+= 1
                        if i == 0: j=1
			(sideC, colC, negC) = Redescription.decoInvStr(alternative[i][0])
                        tmp = currGen[alternative[j][1]][0][sideC].searchTerm(colC, negC)
                        if tmp != None:
			    tmp.update({'side': sideC, 'col': colC})
			    redP = Redescription.fromQueriesPair(currGen[alternative[i][1]][0], self)
                            redE = redP.cousin(self, tmp['side'], -1, tmp['term'])
                    ...
                    for i in range(len(alternative)):
                        if highOp[side][i] != 0:
                            if compQ[0] == None:
                                ...
                                compat &= (currGen[compQ[0][1]][0][side].highOp() == currGen[alternative[i][1]][0][side].highOp())
                                compQ.append(alternative[i])
                        else:
                            compQ.append(alternative[i])
                    compi = 1
                    compOp = currGen[compQ[0][1]][0][side].lowOp()
                    basBuk = currGen[compQ[0][1]][0][side].bukCols()
                    missBas = int(compQ[0][0].split('-')[1])
                    basBuk.append((-1, missBas))
                    basBuk.sort(key=lambda x: x[1])
                    while compat and compi < len(compQ):
                        compBuk = currGen[compQ[compi][1]][0][side].bukCols(compOp)
...
#                            print "(%d) %s (%d) %s, %s" % (ib, basBuk[ib], ic, compBuk[ic], map_buk)
                    ...
                    if compat and cbMiss != None:
                        ...
                        for (b,c) in map_buk.items():
                            if c == cbMiss:
                                bbMiss = b
			(sideC, colC, negC) = Redescription.decoInvStr(compQ[0][0])
                        tmp = currGen[compQ[-1][1]][0][sideC].searchTerm(colC, negC)
                        if tmp != None:
			    tmp.update({'side': sideC, 'col': colC})
			    redP = Redescription.fromQueriesPair(currGen[compQ[-1][1]][0], self)
                            redE = redP.cousin(self, tmp['side'], bbMiss, tmp['term'])
...
#                print "GOOD"
		...
		tmpCurrGen.append((redE.queries,{'acc': redE.acc(), 'opacc': redE.opacc()}))
                for (keyi, i) in alternative:
                    keyRoot = Redescription.invStrQueries(currGen[i][0])
    ...
    def firstGen(self, currGen, idsSets, constraints):
        ...
        if constraints.redLength() == 1:
            for (idS, idsRed) in idsSets.items():
                ...
                idsRed.sort(key=lambda x: currGen[x][0][side].buk[0][0].item)
        
                for idRedA in range(min(20,len(idsRed))):
                    redA = currGen[idsRed[idRedA]]
                    idRedB = idRedA+1
                    tA = currGen[idsRed[idRedA]][0][side].buk[0][0]
                    dA = currGen[idsRed[idRedA]][0][1-side].buk[0][0]

                    while idRedB < len(idsRed) and \
                              ( tA.item.type_id == 1 or tA.item.upb > currGen[idsRed[idRedB]][0][side].buk[0][0].item.lowb ):
                        if dA.col() == currGen[idsRed[idRedB]][0][1-side].buk[0][0].col() :
                            idRedB+=1
                            continue

                        tB = currGen[idsRed[idRedB]][0][side].buk[0][0]
                        dB = currGen[idsRed[idRedB]][0][1-side].buk[0][0]
                        ...
                        if match:
                            ...
                            for op in constraints.queryTypesOp(1-side):
                                for i in range(len(adjust)):
                                    adjust[i]['op'] = op
			    	redP = Redescription.fromQueriesPair(redA[0], self)
                                redE = redP.cousin(self, 1-side, op, dB, adjust)

                                if constraints.isGoodKid(redE, [redA, redB], op):
                                    ind_c = len(tmpCurrGen)
                                    tmpCurrGen.append((redE.queries,{'acc': redE.acc(), 'opacc': redE.opacc()}))
                                    for (keyRoot, t) in [(Redescription.invStrQueries(redA[0]), dB), (Redescription.invStrQueries(redB[0]), dA)]:
