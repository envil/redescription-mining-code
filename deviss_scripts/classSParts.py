...
# compute the advance resulting of appending X on given side with given operator and negation
    ...
    # opposite accuracy
    def opacc(self, side=0):
        lenO = self.lenO( side)
        if lenO == 0:
            return 0
        else:
            return lenO/float(lenO+self.lenD(side))
...
#         return s+r+'||'
    ...
    def parseLPartsChar(parts):
        ...
        partsList = [('acc', '%f',  float(cparts[0])), ('opacc', '%f',  float(cparts[1])), ('pval', '%f',  float(cparts[2]))]
        for i in range(3,len(cparts)):
            partsList.append( (SParts.labels[i-3], '%i',  int(cparts[i])))
        return partsList
    parseLPartsChar = staticmethod(parseLPartsChar)

    def listLPartsChar(self):
        partsList= [('acc', '%f',  self.acc()),('opacc', '%f',  self.opacc()),('pval', '%f',  self.pVal())]
        if self.missing: up_to = SParts.mud
        else: up_to = SParts.delta
        for i in range(up_to+1):
