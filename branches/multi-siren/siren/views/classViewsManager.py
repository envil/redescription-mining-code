...
class VizManager:
    ...
    def clearVizTab(self):
        ...
        for sel in self.vused_ids:
            ### Free another view            
            ## TODO VM
            self.parent.view_ids[self.vused_ids[sel][0]].OnQuit(upMenu=False, freeing=False)
        ...
        for ccid in range(cid+1, self.getVizGridNbDim(dim)+2):
            ...
            for i in range(1, self.getVizGridNbDim(1-dim)+1):
                ...
                if sel in self.vfiller_ids:
                    ...
                    self.parent.view_ids[self.vused_ids[sel][0]].resetGPos(nel)
    ...
    def resizeViz(self):
        if self.isReadyVizIntab():
            ### TODO VM
            for (vid, view) in self.parent.view_ids.items():
                if view.isIntab():
                    view._SetSize()
