...
#        self.mapFrame.Bind(wx.EVT_ENTER_WINDOW, self.OnFocus)
    ...
    def drawMap(self):
        ...
        # m.drawcoastlines(color=ParaView.LINES_COLOR)
        # m.drawcountries(color=ParaView.LINES_COLOR)
        # m.drawmapboundary(fill_color=ParaView.WATER_COLOR) 
        # m.fillcontinents(color=ParaView.GROUND_COLOR, lake_color=ParaView.WATER_COLOR) #'#EEFFFF')
        #     #m.etopo()
    ...
    def updateMap(self, red = None):
        ...
        if red is not None: # and self.coords_proj is not None:
            ...
            draw_settings = {alpha: {"color_e": [i/255.0 for i in colors[0]], "color_f": [i/255.0 for i in colors[0]],
                                     "size": dot_size, "shape": dot_shape, "alpha": ParaView.DOT_ALPHA},
                             ...
                             mua: {"color_e": [i/255.0 for i in colors[0]], "color_f": [0.5,0.5,0.5],
                                     "size": dot_size-1, "shape": dot_shape, "alpha": ParaView.DOT_ALPHA-0.3},
                             ...
                             mud: {"color_e": [0.5,0.5,0.5], "color_f": [0.5, 0.5, 0.5],
                                     "size": dot_size-1, "shape": dot_shape, "alpha": ParaView.DOT_ALPHA-0.3},
                             delta: {"color_e": [0.5,0.5,0.5], "color_f": [0.5, 0.5, 0.5],
                                     "size": dot_size, "shape": dot_shape, "alpha": ParaView.DOT_ALPHA}
                             }
            ...
            for side,pref in [(0,""), (1,"")]:
                ...
                if side == 0:
                    ...
                    if selectids is None:
                        data = [float(a)/topP + 0.125 + (0.2*j)/len(tmp) for j,a in enumerate(tmp)]
                    else:
                        data = [float(osupp[i])/topP + 0.125 + (0.1*j)/len(selectids) for j,i in enumerate(selectids)]
...
# #            self.MapfigMap.canvas.mpl_connect('pick_event', self.OnPick)
    ...
    def getColors(self):
        ...
        colors = []
        for color_k, color in ParaView.colors_def:
            try:
                tmp = t[color_k]["data"]
    ...
    def getDot(self):
        ...
        try:
            ...
            dot_shape = ParaView.DOT_SHAPE
            dot_size = ParaView.DOT_SIZE
