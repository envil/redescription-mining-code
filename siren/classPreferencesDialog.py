...
class PreferencesDialog(wx.Dialog):
	...
	def __init__(self, parent, pm, pv=None):
		"""
		...
		self.pm = pm
		if pv == None:
			self.pv = self.pm.getDefaultTriplets()
		else:
			self.pv = pv
		self.controls_map = {}
		self.objects_map = {}
		...
		for section in self.pm.subsections:
			sec_id = wx.NewId()
			...
			self.setSecValuesFromDict(sec_id, self.pm.getDefaultTriplets())
	...
	def OnPageChanged(self, event):
		...
		if self.controls_map[sec_id]["button"]["apply"].IsEnabled():
			...
			self.pv.update(vdict)
			self.setSecValuesFromDict(sec_id, self.pv)
			self.controls_map[sec_id]["button"]["reset"].Disable()
			self.controls_map[sec_id]["button"]["apply"].Disable()
