        ...
        for red in nextGen :
            ...
            if red.nbAvailableCols() > 0:
                ...
                for pos in bests.improving(minImpr):
                    ...
                    if t.acc() != bests.acc(pos): # and bests.op(pos):
                        #pdb.set_trace()
                        logger.printL(1, 'OUPS!! %s <~> %s %s %f ' % (t, bests.op(pos), bests.term(pos), bests.acc(pos)))
                        #print 'OUPS'
                        #print '%s <~> %s %s %f ' % (t, bests.op(pos), bests.term(pos), bests.acc(pos))
