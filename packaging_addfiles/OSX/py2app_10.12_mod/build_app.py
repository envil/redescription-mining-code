...
class py2app(Command):
    ...
    def filter_out_data(self, dname, fname):
        if re.search("mpl_toolkits/basemap", dname) and re.search("data", fname):
            return True
        if re.search("siren/python-siren", dname) and re.search("data", fname):
            return True
        return False
    ...
    def copy_package_data(self, package, target_dir):
        ...
        for dname in package.packagepath:
            ...
            for fname in filenames:
                if self.filter_out_data(dname, fname):
                    continue
    ...
    def initialize_prescripts(self):
        ...
        if self.site_packages and is_system():
            prescripts.append('system_path_extras')
        prescripts.append('custom_defs')
        #if self.style == 'app':
